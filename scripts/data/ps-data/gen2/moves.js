  ...
  mirrorcoat: {
    ...
    damageCallback: function(pokemon, target) {
      if (
        pokemon.lastAttackedBy &&
        pokemon.lastAttackedBy.thisTurn &&
        this.getCategory(pokemon.lastAttackedBy.move) === "Special" &&
        this.getMove(pokemon.lastAttackedBy.move).id !== "hiddenpower" &&
        target.lastMove !== "sleeptalk"
      ) {
        return 2 * pokemon.lastAttackedBy.damage;
  ...
  mirrormove: {
    ...
    onHit: function(pokemon) {
      ...
      if (
        !foe ||
        !foe.lastMove ||
        (!pokemon.activeTurns && !foe.moveThisTurn) ||
        noMirror[foe.lastMove] ||
        pokemon.moves.indexOf(foe.lastMove) >= 0
      ) {
        return false;
      }
  ...
  roar: {
    ...
    onTryHit: function() {
      for (let i = 0; i < this.queue.length; i++) {
        ...
        if (
          this.queue[i].choice === "move" ||
          this.queue[i].choice === "switch"
        )
          return false;
      }
    },
    ...
    effect: {
      ...
      onTryPrimaryHit: function(target, source, move) {
        ...
        if (move.drain) {
          this.add(
            "-hint",
            "In Gold/Silver/Crystal, draining moves always miss against Substitute."
          );
