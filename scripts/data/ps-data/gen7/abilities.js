...
exports.BattleAbilities = {
  ...
  blaze: {
    ...
    shortDesc:
      "When this Pokemon has 1/3 or less of its max HP, its Fire attacks do 1.5x damage.",
  ...
  damp: {
    desc:
      "While this Pokemon is active, Self-Destruct, Explosion, and the Ability Aftermath are prevented from having an effect.",
    shortDesc:
      "While this Pokemon is active, Self-Destruct, Explosion, and Aftermath have no effect.",
    id: "damp",
  ...
  disguise: {
    ...
    onDamage: function(damage, target, source, effect) {
      if (
        effect &&
        effect.effectType === "Move" &&
        target.template.speciesid === "mimikyu" &&
        !target.transformed
      ) {
    ...
    onEffectiveness: function(typeMod, target, type, move) {
      ...
      if (
        pokemon.template.speciesid !== "mimikyu" ||
        pokemon.transformed ||
  ...
  drizzle: {
    ...
    onStart: function(source) {
      for (let i = 0; i < this.queue.length; i++) {
        if (
          this.queue[i].choice === "runPrimal" &&
          this.queue[i].pokemon === source &&
          source.template.speciesid === "kyogre"
        )
          return;
        if (
          this.queue[i].choice !== "runSwitch" &&
          this.queue[i].choice !== "runPrimal"
        )
          break;
      }
    ...
    onResidual: function(pokemon) {
      ...
      for (let i = 0; i < allyActive.length; i++) {
        if (
          allyActive[i] &&
          allyActive[i].hp &&
          this.isAdjacent(pokemon, allyActive[i]) &&
          allyActive[i].status &&
          this.random(10) < 3
        ) {
    ...
    onModifySpA: function(spa, pokemon) {
      ...
      for (let i = 0; i < allyActive.length; i++) {
        if (
          ...
          allyActive[i].hasAbility(["minus", "plus"])
        ) {
          return this.chainModify(1.5);
        }
      }
  ...
  mummy: {
    desc:
      "Pokemon making contact with this Pokemon have their Ability changed to Mummy. Does not affect the Abilities Multitype or Stance Change.",
    shortDesc:
  ...
  overgrow: {
    desc:
      "When this Pokemon has 1/3 or less of its maximum HP, its attacking stat is multiplied by 1.5 while using a Grass-type attack.",
    shortDesc:
      "When this Pokemon has 1/3 or less of its max HP, its Grass attacks do 1.5x damage.",
    onModifyAtkPriority: 5,
    onModifyAtk: function(atk, attacker, defender, move) {
  ...
  stancechange: {
    ...
    onBeforeMove: function(attacker, defender, move) {
      ...
      if (
        attacker.template.species !== targetSpecies &&
  ...
  swarm: {
    ...
    shortDesc:
      "When this Pokemon has 1/3 or less of its max HP, its Bug attacks do 1.5x damage.",
    onModifyAtkPriority: 5,
    onModifyAtk: function(atk, attacker, defender, move) {
    ...
    onAllyAfterUseItem: function(item, pokemon) {
      ...
      if (
        !this.singleEvent(
          "TakeItem",
          item,
          this.effectData.target.itemData,
          this.effectData.target,
          pokemon,
          this.effectData,
          item
  ...
  torrent: {
    ...
    shortDesc:
      "When this Pokemon has 1/3 or less of its max HP, its Water attacks do 1.5x damage.",
    onModifyAtkPriority: 5,
    onModifyAtk: function(atk, attacker, defender, move) {
  ...
  persistent: {
    shortDesc:
      "The duration of certain field effects is increased by 2 turns if used by this Pokemon.",
    id: "persistent",
    isNonstandard: true,
    name: "Persistent",
