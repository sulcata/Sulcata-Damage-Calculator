...
exports.BattleMovedex = {
  ...
  aromatherapy: {
    ...
    category: "Status",
    desc:
      "Every Pokemon in the user's party is cured of its major status condition. Active Pokemon with the Ability Sap Sipper are not cured, unless they are the user.",
    ...
    onHit: function(pokemon, source, move) {
      ...
      for (let i = 0; i < side.pokemon.length; i++) {
        if (
          side.pokemon[i] !== source &&
          (side.pokemon[i].hasAbility("sapsipper") ||
  ...
  assist: {
    ...
    category: "Status",
    desc:
      "A random move among those known by the user's party members is selected for use. Does not select Assist, Belch, Bestow, Bounce, Chatter, Circle Throw, Copycat, Counter, Covet, Destiny Bond, Detect, Dig, Dive, Dragon Tail, Endure, Feint, Fly, Focus Punch, Follow Me, Helping Hand, Hold Hands, King's Shield, Mat Block, Me First, Metronome, Mimic, Mirror Coat, Mirror Move, Nature Power, Phantom Force, Protect, Rage Powder, Roar, Shadow Force, Sketch, Sky Drop, Sleep Talk, Snatch, Spiky Shield, Struggle, Switcheroo, Thief, Transform, Trick, or Whirlwind.",
    ...
    effect: {
      ...
      onAnyModifyDamage: function(damage, source, target, move) {
        if (target !== source && target.side === this.effectData.target) {
          if (
            (target.side.sideConditions["reflect"] &&
              this.getCategory(move) === "Physical") ||
            (target.side.sideConditions["lightscreen"] &&
  ...
  autotomize: {
    ...
    effect: {
      ...
      onRestart: function(pokemon) {
        if (
          pokemon.template.weightkg - this.effectData.multiplier * 100 >
          0.1
        ) {
  ...
  avalanche: {
    ...
    basePowerCallback: function(pokemon, target, move) {
      if (
        target.lastDamage > 0 &&
        pokemon.lastAttackedBy &&
        pokemon.lastAttackedBy.thisTurn &&
        pokemon.lastAttackedBy.pokemon === target
      ) {
  ...
  banefulbunker: {
    ...
    category: "Status",
    desc:
      "The user is protected from most attacks made by other Pokemon during this turn, and Pokemon making contact with the user become poisoned. This move has a 1/X chance of being successful, where X starts at 1 and triples each time this move is successfully used. X resets to 1 if this move fails or if the user's last move used is not Baneful Bunker, Detect, Endure, King's Shield, Protect, Quick Guard, Spiky Shield, or Wide Guard. Fails if the user moves last this turn.",
  ...
  bounce: {
    ...
    effect: {
      ...
      onAccuracy: function(accuracy, target, source, move) {
        ...
        if (
          source.volatiles["lockon"] &&
          target === source.volatiles["lockon"].source
        )
          return;
        return 0;
      },
  ...
  bugbite: {
    ...
    category: "Physical",
    desc:
      "If this move is successful and the user has not fainted, it steals the target's held Berry if it is holding one and eats it immediately. Items lost to this move cannot be regained with Recycle or the Ability Harvest.",
  ...
  copycat: {
    ...
    category: "Status",
    desc:
      "The user uses the last move used by any Pokemon, including itself. Fails if no move has been used, or if the last move used was Assist, Baneful Bunker, Belch, Bestow, Chatter, Circle Throw, Copycat, Counter, Covet, Destiny Bond, Detect, Dragon Tail, Endure, Feint, Focus Punch, Follow Me, Helping Hand, Hold Hands, King's Shield, Mat Block, Me First, Metronome, Mimic, Mirror Coat, Mirror Move, Nature Power, Protect, Rage Powder, Roar, Sketch, Sleep Talk, Snatch, Spiky Shield, Struggle, Switcheroo, Thief, Transform, Trick, or Whirlwind.",
    ...
    onHit: function(pokemon) {
      ...
      if (
        !this.lastMove ||
        noCopycat[this.lastMove] ||
        this.getMove(this.lastMove).isZ
      ) {
        return false;
      }
  ...
  coreenforcer: {
    ...
    category: "Special",
    desc:
      "If the user moves after the target, the target's Ability is rendered ineffective as long as it remains active. If the target uses Baton Pass, the replacement will remain under this effect. If the target's Ability is Multitype or Stance Change, this effect does not happen.",
  ...
  destinybond: {
    ...
    category: "Status",
    desc:
      "Until the user's next turn, if an opposing Pokemon's attack knocks the user out, that Pokemon faints as well, unless the attack was Doom Desire or Future Sight. Fails if the user used this move successfully last turn.",
  ...
  detect: {
    ...
    category: "Status",
    desc:
      "The user is protected from most attacks made by other Pokemon during this turn. This move has a 1/X chance of being successful, where X starts at 1 and triples each time this move is successfully used. X resets to 1 if this move fails or if the user's last move used is not Baneful Bunker, Detect, Endure, King's Shield, Protect, Quick Guard, Spiky Shield, or Wide Guard. Fails if the user moves last this turn.",
  ...
  electrify: {
    ...
    category: "Status",
    desc:
      "Causes the target's move to become Electric type this turn. Fails if the target already moved this turn.",
  ...
  electroball: {
    ...
    category: "Special",
    desc:
      "The power of this move depends on (user's current Speed / target's current Speed), rounded down. Power is equal to 150 if the result is 4 or more, 120 if 3, 80 if 2, 60 if 1, 40 if less than 1.",
  ...
  encore: {
    ...
    effect: {
      ...
      onStart: function(target) {
        ...
        if (
          !target.lastMove ||
          this.getMove(target.lastMove).isZ ||
          noEncore[target.lastMove] ||
          (target.moveset[moveIndex] && target.moveset[moveIndex].pp <= 0)
        ) {
          // it failed
      ...
      onResidual: function(target) {
        if (
          target.moves.indexOf(target.lastMove) >= 0 &&
          target.moveset[target.moves.indexOf(target.lastMove)].pp <= 0
        ) {
  ...
  entrainment: {
    ...
    onTryHit: function(target, source) {
      ...
      if (
        bannedTargetAbilities[target.ability] ||
        bannedSourceAbilities[source.ability] ||
  ...
  explosion: {
    ...
    category: "Physical",
    desc:
      "The user faints after using this move, even if this move fails for having no target. This move is prevented from executing if any active Pokemon has the Ability Damp.",
  ...
  fairylock: {
    ...
    category: "Status",
    desc:
      "Prevents all active Pokemon from switching next turn. A Pokemon can still switch out if it is holding Shed Shell or uses Baton Pass, Parting Shot, U-turn, or Volt Switch. Fails if this move was used successfully last turn.",
    ...
    onPrepareHit: function(target, source, move) {
      for (let i = 0; i < this.queue.length; i++) {
        ...
        if (
          !decision.move ||
          !decision.pokemon ||
          !decision.pokemon.isActive ||
          decision.pokemon.fainted
        )
          continue;
        if (
          decision.pokemon.side === source.side &&
          ["grasspledge", "waterpledge"].includes(decision.move.id)
        ) {
  ...
  flowershield: {
    ...
    category: "Status",
    desc:
      "Raises the Defense of all Grass-type Pokemon on the field by 1 stage.",
    ...
    onHitField: function(target, source) {
      ...
      for (let sideSlot = 0; sideSlot < this.sides.length; sideSlot++) {
        ...
        for (let activeSlot = 0; activeSlot < sideActive.length; activeSlot++) {
          if (
            sideActive[activeSlot] &&
            sideActive[activeSlot].isActive &&
            sideActive[activeSlot].hasType("Grass")
          ) {
  ...
  frostbreath: {
    ...
    category: "Special",
    desc:
      "This move is always a critical hit unless the target is under the effect of Lucky Chant or has the Abilities Battle Armor or Shell Armor.",
  ...
  gastroacid: {
    ...
    category: "Status",
    desc:
      "Causes the target's Ability to be rendered ineffective as long as it remains active. If the target uses Baton Pass, the replacement will remain under this effect. Fails if the target's Ability is Multitype or Stance Change.",
    ...
    effect: {
      ...
      onStart: function(pokemon) {
        ...
        this.singleEvent(
          "End",
          this.getAbility(pokemon.ability),
          pokemon.abilityData,
          pokemon,
          pokemon,
          "gastroacid"
  ...
  gearup: {
    ...
    category: "Status",
    desc:
      "Raises the Attack and Special Attack of Pokemon on the user's side with the Abilities Plus or Minus by 1 stage.",
    ...
    onHitSide: function(side, source) {
      ...
      for (let i = 0; i < targets.length; i++)
        this.boost({ atk: 1, spa: 1 }, targets[i], source, "move: Gear Up");
    },
    secondary: false,
    target: "allySide",
    type: "Steel",
  ...
  grassknot: {
    ...
    category: "Special",
    desc:
      "Deals damage to the target based on its weight. Power is 20 if less than 10 kg, 40 if less than 25 kg, 60 if less than 50 kg, 80 if less than 100 kg, 100 if less than 200 kg, and 120 if greater than or equal to 200 kg.",
  ...
  grasspledge: {
    ...
    onPrepareHit: function(target, source, move) {
      for (let i = 0; i < this.queue.length; i++) {
        ...
        if (
          decision.pokemon.side === source.side &&
          ["waterpledge", "firepledge"].includes(decision.move.id)
        ) {
          this.prioritizeQueue(decision);
          this.add("-waiting", source, decision.pokemon);
          return null;
  ...
  gravity: {
    ...
    effect: {
      ...
      onStart: function() {
        ...
        const allActivePokemon = this.sides[0].active.concat(
          this.sides[1].active
        );
  ...
  grudge: {
    ...
    category: "Status",
    desc:
      "Until the user's next turn, if a foe's attack knocks the user out, that foe loses all remaining PP for that attack.",
          ...
          for (let i in source.moveset) {
            if (source.moveset[i].id === source.lastMove) {
              ...
              this.add(
                ...
                source,
                "move: Grudge",
                this.getMove(source.lastMove).name
              );
            }
          }
        }
  ...
  guardianofalola: {
    ...
    damageCallback: function(pokemon, target) {
      if (
        ...
        target.side.sideConditions["matblock"] ||
        ...
        this.add(
          "-message",
          target.name +
            " couldn't fully protect itself and got hurt! (placeholder)"
        );
        return this.clampIntRange(
  ...
  healbell: {
    ...
    category: "Status",
    desc:
      "Every Pokemon in the user's party is cured of its major status condition. Active Pokemon with the Ability Soundproof are not cured.",
    shortDesc: "Cures the user's party of all status conditions.",
    id: "healbell",
    isViable: true,
    name: "Heal Bell",
  ...
  healpulse: {
    ...
    category: "Status",
    desc:
      "The target restores 1/2 of its maximum HP, rounded half up. If the user has the Ability Mega Launcher, the target instead restores 3/4 of its maximum HP, rounded half down.",
    shortDesc: "Heals the target by 50% of its max HP.",
    id: "healpulse",
    name: "Heal Pulse",
    pp: 10,
  ...
  healingwish: {
    ...
    category: "Status",
    desc:
      "The user faints and the Pokemon brought out to replace it has its HP fully restored along with having any major status condition cured. Fails if the user is the last unfainted Pokemon in its party.",
  ...
  highjumpkick: {
    ...
    category: "Physical",
    desc:
      "If this attack is not successful, the user loses half of its maximum HP, rounded down, as crash damage. Pokemon with the Ability Magic Guard are unaffected by crash damage.",
  ...
  imprison: {
    ...
    category: "Status",
    desc:
      "The user prevents all of its foes from using any moves that the user also knows as long as the user remains active.",
  ...
  instruct: {
    ...
    category: "Status",
    desc:
      "The target immediately uses its last used move. Fails if the target has not made a move, if the move has 0 PP, if the target is preparing to use Beak Blast, Focus Punch, or Shell Trap, or if the move is Assist, Beak Blast, Bide, Copycat, Focus Punch, Ice Ball, Instruct, Me First, Metronome, Mimic, Mirror Move, Nature Power, Outrage, Petal Dance, Rollout, Shell Trap, Sketch, Sleep Talk, Thrash, Transform, a charge move, a recharge move, or a Z-Move.",
  ...
  laserfocus: {
    ...
    category: "Status",
    desc:
      "On the next turn, the user's attack will be a critical hit. Fails if the user already has the effect.",
  ...
  lockon: {
    ...
    category: "Status",
    desc:
      "On the following turn, the target cannot avoid the user's moves, even if the target is in the middle of a two-turn move. Fails if the user used this move successfully last turn and that target is still active.",
  ...
  luckychant: {
    ...
    category: "Status",
    desc:
      "For 5 turns, the user and its party members cannot be struck by a critical hit. Fails if this move is already in effect for the user's side.",
  ...
  lunardance: {
    ...
    category: "Status",
    desc:
      "The user faints and the Pokemon brought out to replace it has its HP and PP fully restored along with having any major status condition cured. Fails if the user is the last unfainted Pokemon in its party.",
  ...
  magneticflux: {
    ...
    category: "Status",
    desc:
      "Raises the Defense and Special Defense of Pokemon on the user's side with the Abilities Plus or Minus by 1 stage.",
  ...
  magnetrise: {
    ...
    category: "Status",
    desc:
      "For 5 turns, the user is immune to Ground-type attacks and the effects of Spikes, Toxic Spikes, Sticky Web, and the Ability Arena Trap as long as it remains active. If the user uses Baton Pass, the replacement will gain the effect. Ingrain, Smack Down, Thousand Arrows, and Iron Ball override this move if the user is under any of their effects. Fails if the user is already under this effect or the effects of Ingrain, Smack Down, or Thousand Arrows.",
  ...
  metronome: {
    ...
    category: "Status",
    desc:
      "A random move is selected for use, other than After You, Assist, Belch, Bestow, Celebrate, Chatter, Copycat, Counter, Covet, Crafty Shield, Destiny Bond, Detect, Diamond Storm, Endure, Feint, Focus Punch, Follow Me, Freeze Shock, Happy Hour, Helping Hand, Hold Hands, Hyperspace Hole, Ice Burn, King's Shield, Light of Ruin, Mat Block, Me First, Metronome, Mimic, Mirror Coat, Mirror Move, Nature Power, Protect, Quash, Quick Guard, Rage Powder, Relic Song, Secret Sword, Sketch, Sleep Talk, Snarl, Snatch, Snore, Spiky Shield, Steam Eruption, Struggle, Switcheroo, Techno Blast, Thief, Thousand Arrows, Thousand Waves, Transform, Trick, V-create, or Wide Guard.",
  ...
  mimic: {
    ...
    onHit: function(target, source) {
      ...
      if (
        source.transformed ||
        !target.lastMove ||
        disallowedMoves[target.lastMove] ||
        source.moves.indexOf(target.lastMove) >= 0
      )
        return false;
  ...
  minimize: {
    ...
    category: "Status",
    desc:
      "Raises the user's evasiveness by 2 stages. Whether or not the user's evasiveness was changed, Body Slam, Dragon Rush, Flying Press, Heat Crash, Heavy Slam, Phantom Force, Shadow Force, Steamroller, and Stomp will not check accuracy and have their damage doubled if used against the user while it is active.",
  ...
  miracleeye: {
    ...
    category: "Status",
    desc:
      "Causes the target to have its positive evasiveness stat stage ignored while it is active. Psychic-type attacks can hit the target if it is a Dark type. The effect ends when the target is no longer active. Fails if the target is already affected, or affected by Foresight or Odor Sleuth.",
    ...
    effect: {
      ...
      onBoost: function(boost, target, source, effect) {
        if (
          source &&
          target !== source &&
          (!effect.infiltrates || target.side === source.side)
        ) {
          let showMsg = false;
          for (let i in boost) {
            if (boost[i] < 0) {
  ...
  moonlight: {
    ...
    onHit: function(pokemon) {
      if (this.isWeather(["sunnyday", "desolateland"])) {
        ...
        this.isWeather(["raindance", "primordialsea", "sandstorm", "hail"])
      ) {
  ...
  odorsleuth: {
    ...
    category: "Status",
    desc:
      "Causes the target to have its positive evasiveness stat stage ignored while it is active. Normal- and Fighting-type attacks can hit the target if it is a Ghost type. The effect ends when the target is no longer active. Fails if the target is already affected, or affected by Foresight or Miracle Eye.",
    shortDesc: "Fighting, Normal hit Ghost. Evasiveness ignored.",
    id: "odorsleuth",
    name: "Odor Sleuth",
    pp: 40,
  ...
  payback: {
    ...
    category: "Physical",
    desc:
      "Power doubles if the target moves before the user; power is not doubled if the target switches out.",
        ...
        for (let j = 0; j < this.sides[i].active.length; j++) {
          if (this.sides[i].active[j] && this.sides[i].active[j].isActive) {
            if (
              ...
              this.runEvent("TryHit", this.sides[i].active[j], source, move) ===
              null
            ) {
              result = true;
  ...
  powertrick: {
    ...
    category: "Status",
    desc:
      "The user swaps its Attack and Defense stats; stat stage changes remain on their respective stats. This move can be used again to swap the stats back. If the user uses Baton Pass, the replacement will have its Attack and Defense stats swapped if the effect is active. If the user has its stats recalculated by changing forme while its stats are swapped, this effect is ignored but is still active for the purposes of Baton Pass.",
  ...
  present: {
    ...
    category: "Physical",
    desc:
      "Deals damage or heals the target. 40% chance for 40 power, 30% chance for 80 power, 10% chance for 120 power, and 20% chance to heal the target by 1/4 of its maximum HP, rounded down. This move must hit to be effective.",
  ...
  psychoshift: {
    ...
    category: "Status",
    desc:
      "The user's major status condition is transferred to the target, and the user is then cured. Fails if the target already has a major status condition.",
    ...
    onHit: function(target, pokemon) {
      if (
        pokemon.status &&
        !target.status &&
        target.trySetStatus(pokemon.status)
      ) {
        pokemon.cureStatus();
      } else {
        return false;
        ...
        for (let i = 0; i < sources.length; i++) {
          ...
          if (sources[i].canMegaEvo) {
            for (let j = 0; j < this.queue.length; j++) {
              if (
                this.queue[j].pokemon === sources[i] &&
                this.queue[j].choice === "megaEvo"
              ) {
          ...
          this.runMove(
            "pursuit",
            sources[i],
            this.getTargetLoc(pokemon, sources[i])
          );
        }
      }
    },
      ...
      if (move.willChangeForme) {
        if (
          ...
          this.add(
            ...
            pokemon,
            pokemon.illusion
              ? pokemon.illusion.template.species
  ...
  roleplay: {
    ...
    onTryHit: function(target, source) {
      ...
      if (
        bannedAbilities[target.ability] ||
        source.ability === "multitype" ||
        target.ability === source.ability
      ) {
        return false;
      }
  ...
  rototiller: {
    ...
    onHitField: function(target, source) {
      ...
      for (let i = 0; i < targets.length; i++)
        this.boost({ atk: 1, spa: 1 }, targets[i], source);
    },
    secondary: false,
    target: "all",
    type: "Ground",
  ...
  shelltrap: {
    ...
    category: "Special",
    desc:
      "Fails unless the user is hit by a physical attack from an opponent this turn before it can execute the move.",
  ...
  skillswap: {
    ...
    category: "Status",
    desc:
      "The user swaps its Ability with the target's Ability. Fails if either the user or the target's Ability is Illusion, Multitype, Stance Change, or Wonder Guard, but does not fail if both have the same Ability.",
    ...
    onHit: function(target, source, move) {
      ...
      source.battle.singleEvent(
        "End",
        sourceAbility,
        source.abilityData,
        source
      );
      target.battle.singleEvent(
        "End",
        targetAbility,
        target.abilityData,
        target
    ...
    onHit: function(pokemon) {
      ...
      for (let i = 0; i < pokemon.moveset.length; i++) {
        ...
        if (
          ...
          !(
            noSleepTalk[move] ||
            this.getMove(move).flags["charge"] ||
            (this.getMove(move).isZ && this.getMove(move).basePower !== 1)
          )
        ) {
          moves.push(move);
        }
  ...
  spectralthief: {
    ...
    category: "Physical",
    desc:
      "The target's raised stat stages are stolen from it and applied to the user before dealing damage.",
  ...
  spikyshield: {
    ...
    category: "Status",
    desc:
      "The user is protected from most attacks made by other Pokemon during this turn, and Pokemon making contact with the user lose 1/8 of their maximum HP, rounded down. This move has a 1/X chance of being successful, where X starts at 1 and triples each time this move is successfully used. X resets to 1 if this move fails or if the user's last move used is not Baneful Bunker, Detect, Endure, King's Shield, Protect, Quick Guard, Spiky Shield, or Wide Guard. Fails if the user moves last this turn.",
    ...
    onHit: function(target) {
      if (target.deductPP(target.lastMove, 4)) {
        this.add(
          ...
          target,
          "move: Spite",
          this.getMove(target.lastMove).name,
          4
        );
        return;
      }
  ...
  stickyweb: {
    ...
    category: "Status",
    desc:
      "Sets up a hazard on the foe's side of the field, lowering the Speed by 1 stage of each foe that switches in, unless it is a Flying-type Pokemon or has the Ability Levitate. Can be used only once before failing. Can be removed from the foe's side if any foe uses Rapid Spin or Defog, or is hit by Defog.",
  ...
  stockpile: {
    ...
    category: "Status",
    desc:
      "Raises the user's Defense and Special Defense by 1 stage. The user's Stockpile count increases by 1. Fails if the user's Stockpile count is 3.",
  ...
  struggle: {
    ...
    category: "Physical",
    desc:
      "Deals typeless damage to one adjacent foe at random. If this move was successful, the user loses 1/4 of its maximum HP, rounded half up; the Ability Rock Head does not prevent this. This move can only be used if none of the user's known moves can be selected.",
  ...
  suckerpunch: {
    ...
    onTry: function(source, target) {
      ...
      if (
        !decision ||
        decision.choice !== "move" ||
        (decision.move.category === "Status" &&
          decision.move.id !== "mefirst") ||
  ...
  taunt: {
    ...
    category: "Status",
    desc:
      "Prevents the target from using non-damaging moves for its next three turns. Pokemon with the Ability Oblivious or protected by the Ability Aroma Veil are immune.",
  ...
  thunder: {
    ...
    category: "Special",
    desc:
      "Has a 30% chance to paralyze the target. This move can hit a target using Bounce, Fly, or Sky Drop. If the weather is Rain Dance, this move does not check accuracy. If the weather is Sunny Day, this move's accuracy is 50%.",
  ...
  torment: {
    ...
    category: "Status",
    desc:
      "Prevents the target from selecting the same move for use two turns in a row. This effect lasts until the target leaves the field.",
  ...
  trumpcard: {
    ...
    category: "Special",
    desc:
      "This move does not check accuracy. The power of this move is based on the amount of PP remaining after normal PP reduction and the Ability Pressure resolve. 200 power for 0 PP, 80 power for 1 PP, 60 power for 2 PP, 50 power for 3 PP, and 40 power for 4 or more PP.",
  ...
  uproar: {
    ...
    category: "Special",
    desc:
      "Deals damage to one adjacent foe at random. The user spends three turns locked into this move. On the first of the three turns, all sleeping active Pokemon wake up. During the three turns, no active Pokemon can fall asleep by any means; Pokemon switched in during the effect do not wake up. If the user is prevented from moving or the attack is not successful against the target during one of the turns, the effect ends.",
  ...
  waterpledge: {
    ...
    onPrepareHit: function(target, source, move) {
      for (let i = 0; i < this.queue.length; i++) {
        ...
        if (
          decision.pokemon.side === source.side &&
          ["firepledge", "grasspledge"].includes(decision.move.id)
        ) {
          this.prioritizeQueue(decision);
          this.add("-waiting", source, decision.pokemon);
          return null;
  ...
  watersport: {
    ...
    category: "Status",
    desc:
      "For 5 turns, all Fire-type attacks used by any active Pokemon have their power reduced to 0.33x. Fails if this move is already in effect.",
