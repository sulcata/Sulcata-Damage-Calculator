...
exports.BattleItems = {
  ...
  chopleberry: {
    ...
    onSourceModifyDamage: function(damage, source, target, move) {
      ...
      if (
        move.type === "Fighting" &&
        move.typeMod > 0 &&
        (!target.volatiles["substitute"] || move.flags["authentic"])
      ) {
        if (target.eatItem()) {
  ...
  griseousorb: {
    inherit: true,
    desc:
      "Can only be held by Giratina. Its Ghost- & Dragon-type attacks have 1.2x power.",
    onBasePower: function(basePower, user, target, move) {
      if (
        user.template.num === 487 &&
    ...
    effect: {
      ...
      onAfterMoveSecondarySelf: function(source, target, move) {
        if (
          ...
          source &&
          source.volatiles["lifeorb"]
        ) {
          this.damage(
            source.maxhp / 10,
            source,
            source,
            this.getItem("lifeorb")
          );
  ...
  metronome: {
    inherit: true,
    desc:
      "Damage of moves used on consecutive turns is increased. Max 2x after 10 turns.",
    effect: {
  ...
  thickclub: {
    ...
    onModifyAtk: function(atk, pokemon) {
      if (
        pokemon.template.species === "Cubone" ||
        pokemon.template.species === "Marowak"
      ) {
        return this.chainModify(2);
      }
    }
