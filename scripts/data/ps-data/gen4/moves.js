...
exports.BattleMovedex = {
  ...
  brickbreak: {
    inherit: true,
    desc:
      "Whether or not this attack misses or the target is immune, the effects of Reflect and Light Screen end for the target's side of the field before damage is calculated.",
  ...
  disable: {
    ...
    accuracy: 80,
    desc:
      "For 4 to 7 turns, the target's last move used becomes disabled. Fails if one of the target's moves is already disabled, if the target has not made a move, or if the target no longer knows the move.",
    flags: { protect: 1, mirror: 1, authentic: 1 },
  ...
  highjumpkick: {
    ...
    basePower: 100,
    desc:
      "If this attack is not successful, the user loses HP equal to half the target's maximum HP, rounded down, if the target is immune, or half of the damage the target would have taken, rounded down, but no less than 1 HP and no more than half of the target's maximum HP, as crash damage. Pokemon with the Ability Magic Guard are unaffected by crash damage.",
  ...
  imprison: {
    inherit: true,
    desc:
      "The user prevents all of its foes from using any moves that the user also knows as long as the user remains active. Fails if no opponents know any of the user's moves.",
    flags: { authentic: 1 },
    onTryHit: function(pokemon) {
  ...
  mimic: {
    ...
    onHit: function(target, source) {
      ...
      if (
        ...
        source.moves.indexOf(target.lastMove) !== -1 ||
  ...
  payback: {
    ...
    desc:
      "Power doubles if the target moves before the user; power is also doubled if the target switches out."
  },
  petaldance: {
    inherit: true,
    basePower: 90,
