...
exports.BattleMovedex = {
  ...
  conversion: {
    inherit: true,
    desc:
      "The user's type changes to match the original type of one of its four moves besides this move, at random, but not either of its current types. Fails if the user cannot change its type, or if this move would only be able to select one of the user's current types.",
    shortDesc: "Changes user's type to match a known move.",
    onHit: function(target) {
      let possibleTypes = target.moveset
        .map(val => {
  ...
  healbell: {
    inherit: true,
    desc:
      "Every Pokemon in the user's party is cured of its major status condition. Active Pokemon with the Ability Soundproof are also cured.",
  ...
  incinerate: {
    ...
    basePower: 30,
    desc:
      "The target loses its held item if it is a Berry. This move cannot cause Pokemon with the Ability Sticky Hold to lose their held item. Items lost to this move cannot be regained with Recycle or the Ability Harvest.",
  ...
  knockoff: {
    ...
    basePower: 20,
    desc:
      "If the user has not fainted, the target loses its held item. This move cannot cause Pokemon with the Ability Sticky Hold to lose their held item, or force a Giratina, an Arceus, or a Genesect to lose their Griseous Orb, Plate, or Drive, respectively. Items lost to this move cannot be regained with Recycle or the Ability Harvest.",
        ...
        if (
          ...
          if (!move.crit && !move.infiltrates) {
            ...
            if (target.side.active.length > 1)
              return this.chainModify([0xa8f, 0x1000]);
  ...
  minimize: {
    inherit: true,
    desc:
      "Raises the user's evasiveness by 2 stages. Whether or not the user's evasiveness was changed, Stomp and Steamroller will have their power doubled if used against the user while it is active.",
    pp: 20,
    effect: {
      noCopy: true,
  ...
  mudsport: {
    ...
    category: "Status",
    desc:
      "Until the user is no longer active, all Electric-type attacks used by any active Pokemon have their power reduced to 0.33x. Fails if this move is already in effect; not stackable.",
  ...
  quickguard: {
    ...
    effect: {
      ...
      onTryHit: function(target, source, effect) {
        ...
        if (
          effect &&
          (effect.id === "feint" || this.getMove(effect.id).priority <= 0)
        ) {
          return;
        }
  ...
  secretpower: {
    inherit: true,
    desc:
      "Has a 30% chance to cause a secondary effect on the target based on the battle terrain. Lowers accuracy by 1 stage on the regular Wi-Fi terrain. The secondary effect chance is not affected by the Ability Serene Grace.",
  ...
  skillswap: {
    ...
    onHit: function(target, source) {
      ...
      this.add(
        ...
        source,
        "move: Skill Swap",
        this.getAbility(targetAbility),
        this.getAbility(sourceAbility),
        "[of] " + target
      );
  ...
  watersport: {
    ...
    category: "Status",
    desc:
      "Until the user is no longer active, all Fire-type attacks used by any active Pokemon have their power reduced to 0.33x. Fails if this move is already in effect; not stackable.",
