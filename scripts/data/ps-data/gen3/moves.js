...
exports.BattleMovedex = {
  ...
  astonish: {
    ...
    desc:
      "Has a 30% chance to flinch the target. Damage doubles if the target has used Minimize while active."
  },
  beatup: {
    inherit: true,
    desc:
  ...
  counter: {
    ...
    damageCallback: function(pokemon) {
      if (
        ...
        (this.getCategory(pokemon.lastAttackedBy.move) === "Physical" ||
          this.getMove(pokemon.lastAttackedBy.move).id === "hiddenpower")
      ) {
    ...
    effect: {
      ...
      onDamage: function(damage, target, source, effect) {
        if (
          ...
          this.getCategory(effect.id) === "Physical"
        ) {
  ...
  disable: {
    ...
    accuracy: 55,
    desc:
      "For 2 to 5 turns, the target's last move used becomes disabled. Fails if one of the target's moves is already disabled, if the target has not made a move, or if the target no longer knows the move.",
  ...
  encore: {
    inherit: true,
    desc:
      "For 3 to 6 turns, the target is forced to repeat its last move used. If the affected move runs out of PP, the effect ends. Fails if the target is already under this effect, if it has not made a move, if the move has 0 PP, or if the move is Encore, Mirror Move, or Struggle.",
  ...
  extrasensory: {
    ...
    desc:
      "Has a 10% chance to flinch the target. Damage doubles if the target has used Minimize while active."
  },
  fakeout: {
    inherit: true,
    flags: { protect: 1, mirror: 1 }
    ...
    effect: {
      ...
      onDamage: function(damage, target, source, effect) {
        if (
          ...
          effect.effectType === "Move" &&
          source.side !== target.side &&
          this.getCategory(effect.id) === "Special"
        ) {
          this.effectData.position = source.position;
          this.effectData.damage = 2 * damage;
        }
  ...
  mirrormove: {
    ...
    onHit: function(pokemon) {
      ...
      if (
        !pokemon.lastAttackedBy ||
        !pokemon.lastAttackedBy.pokemon.lastMove ||
        noMirror[pokemon.lastAttackedBy.move] ||
  ...
  sleeptalk: {
    ...
    onHit: function(pokemon) {
      ...
      for (let i = 0; i < pokemon.moveset.length; i++) {
        ...
        if (
          move &&
          !(NoSleepTalk[move] || this.getMove(move).flags["charge"])
        ) {
  ...
  spite: {
    inherit: true,
    desc:
      "Causes the target's last move used to lose 2 to 5 PP, at random. Fails if the target has not made a move, if the move has 0 PP, or if it no longer knows the move.",
  ...
  stockpile: {
    inherit: true,
    desc:
      "The user's Stockpile count increases by 1. Fails if the user's Stockpile count is 3.",
  ...
  struggle: {
    ...
    accuracy: 100,
    desc:
      "Deals typeless damage to one adjacent foe at random. If this move was successful, the user takes damage equal to 1/2 the HP lost by the target, rounded down, but not less than 1 HP; the Ability Rock Head does not prevent this. This move can only be used if none of the user's known moves can be selected.",
