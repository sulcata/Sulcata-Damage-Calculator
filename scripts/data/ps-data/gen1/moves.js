...
exports.BattleMovedex = {
    ...
    effect: {
      ...
      onBeforeMove: function(pokemon) {
        if (this.effectData.duration === 1) {
          ...
          let target = this.effectData.sourceSide.active[
            this.effectData.sourcePosition
          ];
  ...
  counter: {
    ...
    damageCallback: function(pokemon, target) {
      ...
      if (
        lastUsedMove &&
        lastUsedMove.basePower > 0 &&
        ["Normal", "Fighting"].includes(lastUsedMove.type) &&
        target.battle.lastDamage > 0 &&
        !this.willMove(target)
      ) {
  ...
  dig: {
    inherit: true,
    desc:
      "This attack charges on the first turn and executes on the second. On the first turn, the user avoids all attacks other than Bide and Swift.",
    basePower: 100,
    effect: {
      duration: 2,
      onLockMove: "dig",
      onAccuracy: function(accuracy, target, source, move) {
        ...
        this.add(
          "-message",
          "The foe " + target.name + " can't be hit underground!"
        );
        return null;
      },
      onDamage: function(damage, target, source, move) {
  ...
  disable: {
    inherit: true,
    desc:
      "The target cannot choose a random move for 0-6 turns. Disable only works on one move at a time and fails if the target has not yet used a move or if its move has run out of PP. The target does nothing if it is about to use a move that becomes disabled.",
    effect: {
      duration: 4,
      durationCallback: function(target, source, effect) {
  ...
  fly: {
    ...
    effect: {
      ...
      onAccuracy: function(accuracy, target, source, move) {
        ...
        this.add(
          "-message",
          "The foe " + target.name + " can't be hit while flying!"
        );
        return null;
      },
      onDamage: function(damage, target, source, move) {
  ...
  haze: {
    inherit: true,
    desc:
      "Eliminates any stat stage changes and status from all active Pokemon.",
  ...
  hyperbeam: {
    inherit: true,
    desc:
      "Deals damage to a target. If this move is successful, the user must recharge on the following turn and cannot make a move, unless the opponent faints or a Substitute is destroyed.",
  ...
  leechseed: {
    ...
    effect: {
      ...
      onAfterMoveSelf: function(pokemon) {
        ...
        let toLeech =
          this.clampIntRange(Math.floor(pokemon.maxhp / 16), 1) * toxicCounter;
  ...
  lightscreen: {
    ...
    category: "Status",
    desc:
      "For 5 turns, the user has double Special when attacked. Removed by Haze.",
  ...
  mimic: {
    inherit: true,
    desc:
      "This move is replaced by a random move on target's moveset. The copied move has the maximum PP for that move. Ignores a target's Substitute.",
  ...
  reflect: {
    ...
    category: "Status",
    desc:
      "The user has doubled Defense. Critical hits ignore this protection. It is removed from the user if it is successfully hit by Haze.",
  ...
  seismictoss: {
    inherit: true,
    desc:
      "Deals damage to the target equal to the user's level. This move can hit Ghost-type Pokemon.",
    ...
    effect: {
      ...
      onTryHit: function(target, source, move) {
        ...
        if (target.volatiles["substitute"]) {
          if (move.recoil) {
            this.damage(
              Math.round(damage * move.recoil[0] / move.recoil[1]),
              source,
              target,
              "recoil"
            );
        ...
        if (!target.lastAttackedBy)
          target.lastAttackedBy = { pokemon: source, thisTurn: true };
  ...
  superfang: {
    inherit: true,
    desc:
      "Deals damage to the target equal to half of its current HP, rounded down, but not less than 1 HP. This move can hit Ghost-type Pokemon.",
