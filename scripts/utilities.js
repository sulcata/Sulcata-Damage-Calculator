...
const parseLine = _.flow(
  ...
  _.head,
  _.split(" "),
  _.over([_.head, _.flow(_.tail, _.join(" "))]),
  _.map(_.trim)
);
...
const exceptions = new Set(["670:5"]);

const isAesthetic = id => {
  const [num, form] = id.split(":");
  return form !== "0" && !exceptions.has(id) && baseFormOnly.has(num);
};
...
export const simplifyPokeIds = _.mapKeys(
  _.flow(_.split(":"), _.take(2), _.join(":"))
);
