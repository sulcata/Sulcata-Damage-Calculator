...
const fileTypes = [
  ...
  },
  {
    ...
    transform: _.flow(simplifyPokeIds, removeAestheticPokes)
  },
  {
    name: "statuses",
...
const parseTranslationFile = _.curry(
  async (locale, { name, files, transform = _.identity }) => {
...
const translations = async () => {
  await Promise.all(
    _.map(async locale => {
      ...
      return Promise.all(_.map(parseTranslationFile(locale), fileTypes));
    }, locales)
  );
};
