...
describe("gscCalculate()", () => {
    ...
    test("sanity check", () => {
        ...
        expect(gscCalculate(snorlax, zapdos, doubleEdge, field)).toEqual([
            153, 153, 154, 155, 156, 156, 157, 158, 158, 159, 160, 160, 161,
            162, 163, 163, 164, 165, 165, 166, 167, 168, 168, 169, 170, 170,
            171, 172, 172, 173, 174, 175, 175, 176, 177, 177, 178, 179, 180
        ]);
        ...
        expect(gscCalculate(umbreon, blissey, doubleEdge, field)).toEqual([
            166, 167, 168, 169, 169, 170, 171, 172, 172, 173, 174, 175, 176,
            176, 177, 178, 179, 179, 180, 181, 182, 182, 183, 184, 185, 186,
            186, 187, 188, 189, 189, 190, 191, 192, 192, 193, 194, 195, 196
        ]);
    });
    ...
    test("pursuit doubles after variance", () => {
        ...
        expect(gscCalculate(tyranitar, misdreavus, pursuit, field)).toEqual([
            97, 97, 97, 98, 98, 99, 99, 100, 100, 101, 101, 101, 102,
            102, 103, 103, 104, 104, 105, 105, 105, 106, 106, 107, 107, 108,
            108, 109, 109, 109, 110, 110, 111, 111, 112, 112, 113, 113, 114
        ]);
        misdreavus.switchedOut = true;
        expect(gscCalculate(tyranitar, misdreavus, pursuit, field)).toEqual([
            194, 194, 194, 196, 196, 198, 198, 200, 200, 202, 202, 202, 204,
            204, 206, 206, 208, 208, 210, 210, 210, 212, 212, 214, 214, 216,
            216, 218, 218, 218, 220, 220, 222, 222, 224, 224, 226, 226, 228
        ]);
    });
